...
final class MainMenu {
	/**
	 * Creates a scanner that is used to handle all of the user input that is taken
	 * in and used by the program
	 */
	private final Scanner scanner = new Scanner(System.in);

	/**
	 * Connects the MainMenu to the rest of the functions from the accounts,
	 * reports, prediction, validation, and storage teams
	 */
	 ...
	 * Clears the screen when the program is being run in the terminal Provides a
	 * clean display for new menu or information screens
	 * 
	 * @author Aaron Madou
	 */
	...
	/**
	 * Provides a buffer for when text needs to be displayed on the console or
	 * terminal before moving on to the next part of the program
	 * 
	 * @author Shazadul Islam
	 */
	private void moveOn() {
	...
	/**
	 * Grabs the user's choice of where they want to navigate to when given a menu
	 * with numerical choices associated with choices within the menu, validates
	 * that input to make sure its a valid choice for the menu being navigated, and
	 * returns the users desired numerical choice of the menu option that they want
	 * to navigate too
	 *
	 * @param numChoices - the number of valid number options the user has
	 *                   (1..numChoices) that correlate to menu choices
		...
		while (true) {
			try {
				...
				if (numChoice < 1 || numChoice > numChoices) {
					System.out.print("Please enter a valid number associated with an option displayed in the menu (1-"
							+ numChoices + ") and then press enter: ");
					continue;
				}
				return numChoice;
	...
	/**
	 * Scans user input for a year, validates that input to make sure its a valid
	 * year, and return the valid year from the user input
	 * 
	...
	enum AccountRecoverState {
		USERNAME_DNE, INCORRECT_ANSWER, SUCCESSFUL_PASSWORD_CHANGE, RETURN_TO_MENU
	}
	 ...
	 * @return AccountRecoverState - an enum that tells us what happened when the
	 *         user tried recovering their account
	 * 
	 * @author Shazadul Islam
	 */
	private AccountRecoverState accountRecover(String usernameRecovering) {
		...
		// reportDisplay.printFixedWidthHeader("Proportional Deficit Plan",
		// BeautifulDisplay.BRIGHT_YELLOW, 70);
		BeautifulDisplay.printGradientHeader("RECOVER PASSWORD", 70);
		System.out.println();
//		BeautifulDisplay.printFullWidthSectionHeader("RECOVER PASSWORD", BeautifulDisplay.BRIGHT_GREEN, 70);
//		System.out.println();
...
//			System.out.println();
	...
	private AccountRecoverState recoveryChangePassword(String usernameChanging, String verifiedSecQuestion,
			String verifiedSecAnswer) {
		// If the user's answer to their account's security question is correct
		while (true) {
			clearConsole();
			...
			if (!(moduleHub.followsPasswordRules(recoverNewPassword))) {
				...
				System.out.println(
						"  1. Try creating a different password for your account, " + usernameChanging + ", again");
			...
			boolean changedPasswordAccepted = moduleHub.resetUserPassword(usernameChanging, verifiedSecAnswer,
					recoverNewPassword);
			...
			if (changedPasswordAccepted) {
				clearConsole();
				System.out.println(
						"The password for the account, " + usernameChanging + ", has been successfully changed.");
	 ...
	 * 
	 * @return loginUsername - If a valid login event happens, then the username of
	 *         that valid login attempt is returned null - If anything other than
	 *         the desired valid login attempt happens, then null is returned
	 * 
	 * @author Shazadul Islam
	 */
	private String handleLogIn() {
		while (true) {
			...
			// If the user wants to try logging in again, jump back to the start of the
			// loop, so that they can try logging in again
				...
				while (true) {
					...
					case USERNAME_DNE:
						...
						if (recoverAgainChoice == 1) {
							clearConsole();
							System.out.println(
									"Please enter the username of the account you are trying to recover and then press enter.");
					...
					case SUCCESSFUL_PASSWORD_CHANGE:
						System.out.print(
								"Press enter when you are ready to return to the login menu and try logging in with your updated password...");
						scanner.nextLine();
	...
	private void handleAccountCreation() {
		while (true) {
			...
			System.out.println(
					"Please enter the number associated with the question that you want to be your security question and then press enter: ");
			...
			System.out.println("The security question you chose was: " + BeautifulDisplay.BOLD + registerSecretQuestion
					+ BeautifulDisplay.RESET);
			...
			if (surety == 1) {
				...
				boolean isValidAccount = moduleHub.registerUser(registerUsername, registerPassword,
						registerSecretQuestion, registerSecretAnswer, true);
		...
		while (true) {
			...
			case 1:
				...
				while (uploadingFile) {
					...
					// System.out.print(BeautifulDisplay.RESET);
					...
					if (year == null || year < 1900 || year > 2100) {
						...
						// System.out.println("Would you like to try uploading your CSV file again or
						// return to the Finances Menu?");
					...
					if (result.hasErrors()) {
						...
						while (viewingErrors) {
							...
							System.out.println();
							System.out
									.println("We found " + result.getErrorMessages().size() + " error(s) in your CSV.");
					...
					if (hasDuplicateWarning) {
						...
						if (dupChoice == 2) {
							clearConsole();
							System.out.println(
									"Import cancelled. Please clean your CSV file to remove duplicates and try again.");
							moveOn();
							uploadingFile = false;
	 ...
	 * Reports Menu
	 * 
	 * @return Determines if user wants to go to the back to Finances Menu or the
	 *         Main Menu
	 * @param currentUser - username of the currently signed-in user
	 * 
	 * @author Aaron Madou
	 ...
	 * Predictions Menu
	 * 
	 * @return Determines if user wants to go to the back to the Finances Menu or
	 *         the Main Menu
	 * @param currentUser - username of the currently signed-in user
	 * 
	 * @author Aaron Madou
		...
		while (true) {
			...
			while (isWorkingWithYear) {
				...
				case 5: {
					...
					System.out.println(
							"Choose a category to see how much you can safely spend there without going into deficit:\n");
					for (int i = 0; i < categories.size(); i++) {
						System.out.println(" " + (i + 1) + ". " + categories.get(i));
					}
					System.out.println();
		...
		while (true) {
			...
			case 1:
				...
				while (true) {
					...
					System.out.println();
					System.out.println(
							"Please enter the year of the CSV file you would like to delete below and then press enter. ");
					if (plzEnterYearInRangeDel) {
						System.out.println("Please make sure that the year you enter is between 1900 and 2100.");
						plzEnterYearInRangeDel = false;
					} else if (plzEnterValidYearDel) {
						System.out.println(
								"Please make sure that the year you enter is digits only(e.g., 1999 or 2024).");
						plzEnterValidYearDel = false;
					}
					System.out.println(
							"If you have decided not to delete a CSV file anymore, just press enter without any input.");
	...
	private AccountRecoverState decideChangePassword(String usernameChanging, String oldCurrentPassword) {
		...
		while (true) {
			...
			if (!(moduleHub.followsPasswordRules(decideNewPassword))) {
				...
				System.out.println(BeautifulDisplay.BRIGHT_YELLOW + "  3." + BeautifulDisplay.BRIGHT_YELLOW
						+ " Exit the application");
			...
			boolean changedPasswordAccepted = moduleHub.changePassword(usernameChanging, oldCurrentPassword,
					decideNewPassword);
	...
	private void handleChangePassword(String currentUsername) {
		while (true) {
			...
			System.out.print(BeautifulDisplay.BOLD + "  Current Password:" + BeautifulDisplay.RESET + " ");
	...
	/**
	 * Handles the changing of the user's security question and security question
	 * answer
	 * 
	 * @param currentUsername - User's username
	 * 
		...
		while (true) {
			...
			if (validPass) {
				while (true) {
					...
					boolean updatedSecurity = moduleHub.updateUserSecretQuestionAndAnswer(currentUsername,
							newSecretQuestion, newSecretAnswer);
	 ...
	 *
	 * @param currentUser - username of the currently signed-in user
	 * @return Determines whether or not the user has decided to delete their
	 *         account If the function returns true, then the user has not deleted
	 *         their account and they can return to the Main Menu If the function
	 *         returns false, then the user has deleted their account and are
	 *         forcibly moved to the login page
	 * 
	 * @author Shazadul Islam
	 */
	...
	/**
	 * Runs the entire application with the MainMenu as the main interface for the
	 * user to interact with Makes calls to other menus that handle other parts of
	 * the application Also lets the user log out and quit the program
	 * 
	 * @param args - allows the program to receive and process command-line
	 *             arguments when it is executed in terminal (we have no use for
	 *             this)
	 * 
	 * @author Shazadul Islam
	 */
	...
	/**
	 * When the user requests to exit the application, this function is called to
	 * clean up the scanner, relay a message to the user that have exited the
	 * program, and returns the user to the normal command line in terminal
	 * 
	 * @author Shazadul Islam
	 */
