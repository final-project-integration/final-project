...
final class MainMenu {
	...
	private AccountRecoverState decideChangePassword(String usernameChanging ) {
		//If the user's answer to their account's security question is correct
		while (true) {
			clearConsole();
			//Let them enter a new password
			...
			boolean changedPasswordAccepted = moduleHub.resetUserPassword(usernameChanging, recoverNewPassword);
	...
	private void handleChangePassword(String currentUsername) {
		while (true) {
			...
			if (validPass) {
				AccountRecoverState currentPassChange = decideChangePassword(currentUsername, );
